...
local lshift = function( a, by )
--    assert( a <= math.pow( 2, 32 ), "bad a" ) -- ###1
    assert( by <= 32, "bad by" ) -- ###1
    local setMask = math.pow( 2, by )
    local readMask = 1
    local c = 0
    local cnt = 32 - by
    while( cnt > 0 ) do
        if LrMath.bitAnd( a, readMask ) == readMask then
            c = LrMath.bitOr( c, setMask )
        else
            c = LrMath.bitAnd( c, LrMath.bitXor( setMask, 4294967295 ) )
        end
        setMask = setMask * 2
        readMask = readMask * 2
        cnt = cnt - 1
...
band = function( a, b )
    if b == nil then
        return math.mod( a, 4294967296 )
    end
    if a > 4294967295 then
        app:callingError( "big a" )
    end
    return( LrMath.bitAnd( a, b or 4294967295 ) )
end
bxor = function( ... )
    local p = { ... }
    local c = 0
    ...
    --Debug.pause( bword:getString( c ) )
    return c
end
rrotate = function( a, by ) -- bit32.rrotate
--    if a > math.pow( 2, 32 ) then
--        app:callingError( "bad a: " .. tostring( a ) .. ", max: " .. math.pow( 2, 32 ) ) -- ###1
--    end
    assert( by <= 32, "bad by" ) -- ###1
    local c = LrMath.bitXor( rshift( a, by ), lshift( a, 32 - by ) ) -- single loop would be more efficient.
    return c
end
rshift = function( a, by ) -- bit32.rshift
--    assert( a <= math.pow( 2, 32 ), "bad a" ) -- ###1
    assert( by <= 32, "bad by" ) -- ###1
    local setMask = math.pow( 2, by )
    local readMask = 2147483648
    local c = 0
    local cnt = 32 - by
    while( cnt > 0 ) do
        if LrMath.bitAnd( a, readMask ) == readMask then
            ...
            c = LrMath.bitAnd( c, LrMath.bitXor( setMask, 4294967295 ) ) -- bnot => bclr.
        end
        setMask = setMask / 2
        readMask = readMask / 2
        cnt = cnt - 1
...
bnot = function( a ) -- bit32.bnot
--    assert( a <= math.pow( 2, 32 ), "bad a" ) -- ###1
    return LrMath.bitXor( a, 4294967295 ) -- 32 bits, all 1's.
end
